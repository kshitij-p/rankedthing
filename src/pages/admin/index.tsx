import { type PotentialClip, UserRole } from "@prisma/client";
...
const ClipApprovalPanel = () => {
  ...
  const handleClipMutate = async ({ id }: { id: PotentialClip["id"] }) => {
  ...
  const { mutate: rejectClip } = api.videoClip.rejectPotentialClip.useMutation({
    onMutate: handleClipMutate,
    onSettled: handleClipSettled,
            ...
            allPotentialClips.map((clip) => {
              return (
                <m.div
                  ...
                  exit={{ transform: "translateX(-10%)", opacity: 0 }}
                >
